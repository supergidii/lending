...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for status in status_counts:
            ...
            self.stdout.write(f"Total Amount: Ksh{self.format_amount(status['total_amount'])}")
            self.stdout.write(f"Average Amount: Ksh{self.format_amount(status['avg_amount'])}")
        ...
        self.stdout.write(f'Total Payment Amount: Ksh{self.format_amount(payment_stats["total_amount"])}')
        self.stdout.write(f'Average Payment Amount: Ksh{self.format_amount(payment_stats["avg_amount"])}')
        ...
        self.stdout.write(f'Total Queue Amount: Ksh{self.format_amount(queue_stats["total_amount"])}')
        self.stdout.write(f'Average Queue Amount: Ksh{self.format_amount(queue_stats["avg_amount"])}')
            ...
            if user_investments.exists():
                ...
                for status in ['pending', 'matured', 'paired', 'completed']:
                    ...
                    if status_investments.exists():
                        ...
                        self.stdout.write(f"{status.title()} Investments: {status_investments.count()} (Total: Ksh{self.format_amount(total)})")
                ...
                if payments_made.exists():
                    ...
                    self.stdout.write(f"Payments Made: {payments_made.count()} (Total: Ksh{self.format_amount(total_sent)})")
                
                if payments_received.exists():
                    total_received = payments_received.aggregate(total=Sum('amount'))['total']
                    self.stdout.write(f"Payments Received: {payments_received.count()} (Total: Ksh{self.format_amount(total_received)})")
